...
- **참고 작품**: 위키문헌·공유마당·구텐베르크·문화공공데이터광장 12종에서 모은 문학 자료 **115,326건**(중복 제외) 중 필요한 예문을 찾아 인용하고 더 읽을 작품·책을 추천. 본문이 있는 위키문헌 작품은 3,859편(시계열 2,822 + 발표 시기 미상 서재 1,037)
...
|---|---|---|---|---|
| 위키문헌 시계열 | 2,822편 | 있음 (글자 수) | 발표·창작 연월일 (문서 근거) | Firestore `data` |
| 내가 쓴 기록 | 직접 추가 | 있음 | 작성일 | Firestore `data` |
| 위키문헌 서재 | 1,037편 | 있음 | **발표 시기 미상** (별도 막대) | Firestore `library` |
...
| 번역원 번역출간도서 (공공데이터광장) | 9,840건 (원작 단위) | 서지 정보 | 연도 정보 없음 → 시기 미상 | `data/extra_lib046.json` |
| 번역원 전문도서관 소장 (공공데이터광장) | 7,729건 (원작 단위) | 서지 정보 | 연도 정보 없음 | `data/extra_lib047.json` |
| 국립중앙도서관 소장자료 (공공데이터광장) | 61,212건 (KDC 800번대) | 서지 정보 | 연도 정보 없음 | `data/extra_nltot.json` |
| 올림픽공원 도서정보 (공공데이터광장) | 1,558건 | 서지 정보 | 서지의 발행 연도 | `data/extra_kscd0820181.json` |
| 국립어린이청소년도서관 사서추천 | 748건 | 서지 정보 | 연도 정보 없음 | `data/extra_nlcfsase.json` |
| 국립중앙도서관 사서추천 | 224건 | 서지 정보 | 연도 정보 없음 | `data/extra_nlkf0201.json` |
| 국립세종도서관 사서추천 | 193건 | 서지 정보 | 소개글의 발행 연도 | `data/extra_nlsf0401.json` |
| 청소년권장도서 · 대학신입생추천도서 | 119건 · 4건 | 서지 정보 | 연도 정보 없음 | `data/extra_kpef0102.json`, `extra_kpef0103.json` |
| 국립민속박물관 발간도서 | 1건 | 서지 정보 | 발간 연도 | `data/extra_nfmbook.json` |
| **합계 (중복 제외)** | **115,326건** | | 시기 확인 27,534 · 시기 미상 87,792 | |
...
1. **위키문헌 작품 2,822편** — [한국어 위키문헌](https://ko.wikisource.org) MediaWiki API로 수집 ([`backend/scripts/import_wikisource.py`](backend/scripts/import_wikisource.py))
   - **범위 (네 갈래로 찾고 한곳에서 거름)**
   ...
   - **신뢰성 규칙**
     ...
     - **저작권**: 한국 저작권법상 보호 기간이 끝난 **1962년 이전 사망 작가**의 작품만 수록. 위키문헌 라이선스 틀과 관계없이 1963년 이후 사망한 작가·공동 작가·**번역자·번역 원작자**의 작품(예: 이원수 번역 「왕자와 거지」, 이원수 작사 「고향의 봄」, 원작자가 1967년 사망한 「낸의 비극」)과 위키문헌 사용자가 최근 옮긴 번역문(CC BY-SA)은 제외 — 24편
   - **실존 검증**: 전 작품을 문서 번호(pageid)로 다시 조회 → 시계열 2,822편 + 서재 1,037편 = 3,859편 모두 존재·제목 일치 ([`docs/data-verification.md`](docs/data-verification.md), `python -m scripts.verify_works`)
   - 요청 제한을 지키려고 요청 간격 1.5초 + `Retry-After` 재시도, 위키미디어 정책에 맞는 User-Agent(연락처 포함), 응답은 로컬 캐시(git 제외)
   - 장르 분포: 시 1,508 · 수필 396 · 소설 336 · 단편소설 218 · 시조 107 · 동화 63 · 기타 56 · 장편소설 47 · 평론 15 · 한시 13 · 서간 13 · 중편소설 13 · 희곡 12 · 노래 12 · 가사 11 · 고전시가 2
2. **내가 쓴 기록** — 웹 화면의 '기록 관리' 탭이나 `POST /api/data`로 추가 (`source = "직접 작성"`). 본문을 붙여넣으면 전문이 함께 저장되어 AI가 `read_work`로 읽고 퇴고를 도울 수 있음
3. **참고 작품 서재 1,037편 (발표 시기 미상)** — 장르: 시 239 · 시조 226 · 노래 226 · 한시 62 · 수필 62 · 기타 61 · 소설 50 · 고전시가 38 · 서간 20 · 가사 17 · 단편소설 15 · 판소리 7 · 평론 4 · 희곡 4 · 장편소설 3 · 동화 3
...
7. **문화공공데이터광장 보충 자료 11종** ([`import_kcisa_extra.py`](backend/scripts/import_kcisa_extra.py), 서비스 키는 `backend/.env`의 `KCISA_KEY_<이름>`에만) — 전 쪽을 받아(동시 3개 요청, 이어받기) 문학 자료만 골라 자료별 파일 `data/extra_<이름>.json`으로 저장하고, **웹사이트에서 자료마다 따로 구분**(통계 범례·색·출처표, 기록 관리의 출처 목록)
   | 자료 | 원자료 | 문학 선별 | 고르는 방법 |
   |---|---|---|---|
   | 한국문학번역원 번역출간도서 (API_LIB_046) | 167,658 | 9,844 → 통계 9,840 | 전부 한국 문학 번역서. 2022년 일괄 등록분 16만 건은 **같은 기록이 평균 28번 반복**(서로 다른 번역서 약 5,600종)이라 **여러 언어 번역을 원작 한 편으로 묶고** 번역서 수·언어를 메모에 |
   | 번역원 번역전문도서관 소장 (API_LIB_047) | 122,434 | 7,882 → 7,729 | 위와 같음 (`번역 제목 = 원제` 에서 원제·한국어 저자 추출), 046에 있는 원작은 제외 |
   | 국립중앙도서관 소장자료 | 649,358 | 61,771 → 61,212 | 한국십진분류 **KDC 800번대(문학)**: 둘째 자리 = 나라(한국 38,161 · 영미 6,801 · 일본 5,166 …), 셋째 자리 = 형식(시·희곡·소설·수필…), 801 이론·809 문학사 = 평론 |
   | 올림픽공원 도서정보 | 17,230 | 1,698 → 1,558 | 제목의 문학 형식어(장편소설·시집·그림책…), 서지 문자열 `시공사,2015` 의 발행 연도 |
   | 국립어린이청소년도서관 사서추천 | 7,093 | 759 → 748 | 제목·소개글의 뚜렷한 형식어 (동화·그림책·소설·동시집) — 소개글은 기관이 쓴 글이라 저장하지 않고 링크로만 |
   | 국립중앙도서관 사서추천 | 3,492 | 243 → 224 | 주제 분류 '문학·어문학' 또는 형식어 |
   | 국립세종도서관 사서추천 | 733 | 237 → 193 | 소개글 속 청구기호 KDC 8xx, `출판사 ｜ 2021` 의 발행 연도 |
   | 청소년권장도서 · 대학신입생추천도서 | 831 · 179 | 127 · 8 | 제목·소개글의 형식어 ('에세이'는 과학·경제 에세이가 많아 쓰지 않음) |
   | 국립민속박물관 발간도서 | 24,110 | 1 | 설화·민요 자료집만 — 대부분 연구서·전시 자료이고, 남은 1종도 같은 조사 보고서가 22번 중복 등록 |
   | 국립중앙도서관 OAK-PORTAL | 3,612,537 | 0 | 학술 논문 리포지터리, 응답 시간 초과 반복 → 제외 |
   - **연도**: 대부분 발행 연도 필드가 없어, 형식이 분명한 발행 연도(서지 문자열·소개글의 `출판사 ｜ 연도`)만 쓰고 기관 등록일(`regDate`)은 쓰지 않음 → 나머지는 '시기 미상' 막대로 (소개글 속 작가 생년이 발행 연도로 읽히는 문제를 확인해 소개글 속 일반 연도는 쓰지 않음)
...
| 통합 통계 — 공공데이터 보충 자료가 앞 순위 출처와 겹침 | 번역원 157 · 사서추천·권장도서 86 · 올림픽공원 140 · 국립중앙도서관 소장 559 · KCISA 859 | 앞 순위 출처 쪽만 셈 (자료별 표는 중복 보고서) |
...
| GET | `/api/catalog` | 참고 자료 통합 검색 (공유마당·구텐베르크·공공데이터 보충 자료 11종·KCISA, `source`(통계의 출처 키)·`genre`·`q`) |
...
| `search_books` | 더 읽어 볼 작품·책(시집·소설집·희곡 대본·평론집 등)을 추천할 때 | 공유마당·구텐베르크·공공데이터 보충 자료(번역원 번역서·사서추천·국립중앙도서관 소장 등)·KCISA 참고 자료를 제목·저자·발행처·요약으로 검색 (출처별로 좁히기 가능, 목록 정보와 링크) |
...
- get_data_summary({}) → 성공: { "period": "1447-01-01 ~ 2026-09-24", "count": 2823, "metrics": { "total": 23274359, "average": 8244.5, "median": 384.0, "max": 640707, "min": 22, "std": 33793.5 }, "trend": "하락 (최근 30건 평균 7,220자, 직전
...
- search_works({"keyword": "고향", "genre": "시", "limit": 2}) → 성공: { "total": 61, "items": [ { "id": "KM1OvInWRgd7rrMMuyn9", "date": "1931-11-01", "title": "고향", "author": "박용철", "genre": "시", "value": 207, "source": "위키문헌", "memo": "《고향》 박용철 — 발표 월 기준 · 《박용철 …
...
- get_data_summary({}) → 성공: { "period": "1447-01-01 ~ 2026-09-24", "period_start": "1447-01-01", "period_end": "2026-09-24", "count": 2823, "metrics": { "total": 23274359, "average": 8244.5, "median": 384.0, "max": 640707, "min"
- get_statistics({"group": "decade", "genre": "시"}) → 성공: { "group": "decade", "filters": { "genre": "시" }, "series": [ { "period": "1480년대", "count": 3, "total": 534, "average": 178.0 }, { "period": "1890년대", "count": 1, "total": 165, "average": 165.0 }, { 
...
- **작은 데이터로 먼저 검증**: Firestore에 `seed_firestore --limit 20`으로 20건만 넣고 채팅 2회(요약 질문, 퇴고 요청)로 흐름을 확인한 뒤 `--reset`으로 전체 적재 (현재 data 2,823건 · library 1,037건)
...
|---|---|---|---|
| 1 | 시계열 분석 → 요약 → 서비스 활용 | 위키문헌 2,822편 + 내 기록을 날짜순으로 정렬해 통계·추세를 계산하고(모든 출처 11만 5천여 건은 통합 통계로 함께 표시), 그 요약을 채팅 프롬프트·요약 패널·통계 탭이 함께 씀 | §2, [`summary_service.py`](backend/app/services/summary_service.py), [`docs/data-analysis.md`](docs/data-analysis.md) |
...
| 100개 이상 시계열 + 요약 정보 | 2,823건 (전 작품 위키문헌 실존 재조회: [`docs/data-verification.md`](docs/data-verification.md)), `GET /api/data/summary` |
...
- 출처별 원본 목록과 퍼블릭 도메인 작품의 전체 본문은 [`sources/`](sources/) 폴더에 있습니다. 공개 전에 API 키·개인정보(원 서지의 저자 칸에 들어 있던 개인 이메일 등)가 없는지 점검했고, 추천도서 소개글처럼 기관이 쓴 글은 싣지 않았으며, 구텐베르크 본문은 라이선스 조건대로 원본 파일을 그대로 두었습니다.
