...
- **참고 작품**: 위키문헌·공유마당·구텐베르크·문화공공데이터광장 12종에서 모은 문학 자료 **115,346건**(중복 제외) 중 필요한 예문을 찾아 인용하고 더 읽을 작품·책을 추천. 본문이 있는 위키문헌 작품은 3,873편(시계열 2,834 + 발표 시기 미상 서재 1,039)
...
| 번역원 전문도서관 소장 (공공데이터광장) | 7,730건 (원작 단위) | 서지 정보 | 연도 정보 없음 | `data/extra_lib047.json` |
| 국립중앙도서관 소장자료 (공공데이터광장) | 61,217건 (KDC 800번대) | 서지 정보 | 연도 정보 없음 | `data/extra_nltot.json` |
...
7. **문화공공데이터광장 보충 자료 11종** ([`import_kcisa_extra.py`](backend/scripts/import_kcisa_extra.py), 서비스 키는 `backend/.env`의 `KCISA_KEY_<이름>`에만) — 전 쪽을 받아(동시 3개 요청, 이어받기) 문학 자료만 골라 자료별 파일 `data/extra_<이름>.json`으로 저장하고, **웹사이트에서 자료마다 따로 구분**(통계 범례·색·출처표, 기록 관리의 출처 목록)
   ...
   | 번역원 번역전문도서관 소장 (API_LIB_047) | 122,434 | 7,882 → 7,730 | 위와 같음 (`번역 제목 = 원제` 에서 원제·한국어 저자 추출), 046에 있는 원작은 제외 |
   | 국립중앙도서관 소장자료 | 649,358 | 61,771 → 61,217 | 한국십진분류 **KDC 800번대(문학)**: 둘째 자리 = 나라(한국 38,161 · 영미 6,801 · 일본 5,166 …), 셋째 자리 = 형식(시·희곡·소설·수필…), 801 이론·809 문학사 = 평론 |
   | 올림픽공원 도서정보 | 17,230 | 1,698 → 1,558 | 제목의 문학 형식어(장편소설·시집·그림책…), 서지 문자열 `시공사,2015` 의 발행 연도 |
   | 국립어린이청소년도서관 사서추천 | 7,093 | 759 → 748 | 제목·소개글의 뚜렷한 형식어 (동화·그림책·소설·동시집) |
...
| 통합 통계 — 공공데이터 보충 자료가 앞 순위 출처와 겹침 | 번역원 156 · 사서추천·권장도서 86 · 올림픽공원 140 · 국립중앙도서관 소장 554 · KCISA 859 | 앞 순위 출처 쪽만 셈 (자료별 표는 중복 보고서) |
...
|---|---|---|---|
| 1 | 시계열 분석 → 요약 → 서비스 활용 | 위키문헌 2,834편 + 내 기록을 날짜순으로 정렬해 통계·추세를 계산하고(모든 출처 11만 5천여 건은 통합 통계로 함께 표시), 그 요약을 채팅 프롬프트·요약 패널·통계 탭이 함께 씀 | §2, [`summary_service.py`](backend/app/services/summary_service.py), [`docs/data-analysis.md`](docs/data-analysis.md) |
